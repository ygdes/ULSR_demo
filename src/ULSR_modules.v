...
module ULSR_input_ctrl(
    ...
    output wire D_N);
  wire t1, t2, t3;
  sg13_dfrbp_1 DFF1(.Q(D),  .Q_N(D_N), .D(D_N), .RESET_B(SC), .CLK(SD));  // SD => output 1, SD SD => output 0
  sg13_dfrbp_1 DFF2(.Q(t2), .Q_N(t1),  .D(t1),  .RESET_B(SC), .CLK(D_N));  // needs 4 pulses on SD to trigger the update
